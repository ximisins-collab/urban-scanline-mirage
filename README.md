...
- 绝大多数线条应当发丝般细、连续而修长、排列紧密、对比度低，几乎不使用黑色间隙。
- 相邻色层可以轻微融合，但必须保持细微行层定义，不能变成柔软的横向运动模糊。
- 中等宽度色带只承担次级节奏，宽亮带、暗带和强调条必须极少。
- 每个色域独立控制线条的间距、相位、对比度、起止位置、断点、融合与渐隐，线条应被色域边界裁切，不能让一张机械网格贯穿全图。
- 天空、远景、道路和大墙面保持低对比、低信息密度。
- 建筑、植物、市场或密集街景可以拥有更丰富的层次，但仍应比原图安静。
- 色块边界保持清楚、平直；色块内部需要有光学层次，不能像一块无变化的纯色矢量面。

目标不是“线越多越有风格”，而是让线条承担色彩采样、空间压缩和材质过渡。缩小看时，观者应该先看到安静的大色域；放大后，才发现色域内部密集而细腻的水平层理。

### 四级视觉层次

| 层级 | 画面作用 | 使用原则 |
| --- | --- | --- |
| 大色域 | 决定缩略图中的第一视觉印象 | 少量、宽阔、色彩明确，避免被大量窄柱切碎 |
| 水平微层理 | 构成色域的内部材料 | 最密集、最纤细、低对比、长线为主，近看才明显 |
| 中等色带 | 表示楼层、卷帘门、地平线和压缩后的空间节奏 | 数量有限，集中出现，不平均铺满画面 |
| 宽条与竖向锚点 | 提供少量强调色和场所识别线索 | 极少使用，只保留真正承担结构作用的部分 |

竖向元素不是越多越能说明空间。当树干、柱子、立面边缘被逐一保留并形成真实透视走廊时，应把重复竖条重新合并成更大的色域，只留下最低限度的空间锚点。
...
| 建立肌理 | 如何避免纯色块或统一扫描线？ | 长而纤细的局部微层理，近邻色变化，以及有限的中宽色带 |
| 双尺度检查 | 线条是否既有肌理又不过度抢眼？ | 缩小看大色域与人物，放大看线条清晰度、局部裁切和细微色差 |
...
9. 缩略图中首先读到人物和大色域；原尺寸查看时，能够看见清晰、纤细、局部限定的水平色层，而不是模糊拖影。
...
降低柔化和连续涂抹感，恢复发丝级行层定义。线条可以低对比并相互融合，但相邻色层仍需保持轻微而清晰的色阶变化。

### 画面出现太多柱子或树干

这通常说明场景被重新写实化了。把重复竖向结构合并为更宽阔的建筑、植物或阴影色域，只保留能够说明场所方向的少量竖向锚点。

### 缩小后首先看到一条条横线

线条对比度或黑色间隙过强。降低线条之间的明暗差，让大色域重新成为第一视觉层级，同时保留放大后可见的微层理。
